...
//整数字段：数值字段通过校验
TEST_F(Data_Validator_Test, 整数字段通过)
{
	//含整数字段的配置
...
//整数字段：文本内容被拒绝
TEST_F(Data_Validator_Test, 整数字段拒绝文本)
{
	//整数字段被填成文本
	nlohmann::json config = nlohmann::json::object();
	config["数量"] = "五";
...
//整数字段：浮点内容被拒绝
TEST_F(Data_Validator_Test, 整数字段拒绝浮点)
{
	//整数字段被填成浮点数
	nlohmann::json config = nlohmann::json::object();
	config["数量"] = 1.5;
...
//浮点字段：小数内容通过校验
TEST_F(Data_Validator_Test, 浮点字段通过)
{
	//含浮点字段的配置
	nlohmann::json config = nlohmann::json::object();
	config["速度"] = 1.5;
...
//浮点字段：整数内容被拒绝
TEST_F(Data_Validator_Test, 浮点字段拒绝整数)
{
	//浮点字段被填成整数
	nlohmann::json config = nlohmann::json::object();
	config["速度"] = 1;
...
//字符串字段：非空文本通过校验
TEST_F(Data_Validator_Test, 字符串字段通过)
{
	//含文本字段的配置
	nlohmann::json config = nlohmann::json::object();
	config["名称"] = "引擎";
...
//字符串字段：数值内容被拒绝
TEST_F(Data_Validator_Test, 字符串字段拒绝数值)
{
	//文本字段被填成数值
	nlohmann::json config = nlohmann::json::object();
	config["名称"] = 9;
...
//容器字段：非空数组通过校验
TEST_F(Data_Validator_Test, 数组字段通过)
{
	//含数组字段的配置
	nlohmann::json config = nlohmann::json::object();
...
TEST_F(Data_Validator_Test, 空数组字段被拒绝)
{
	//数组字段被留空
	nlohmann::json config = nlohmann::json::object();
...
//嵌套对象字段：非空对象通过校验
TEST_F(Data_Validator_Test, 嵌套对象字段通过)
{
	//含嵌套对象的配置
	nlohmann::json config = nlohmann::json::object();
...
//缺失字段：直接判定失败
TEST_F(Data_Validator_Test, 缺失字段被拒绝)
{
	//空配置对象
	nlohmann::json config = nlohmann::json::object();
	//不存在的字段校验应失败
...
TEST_F(Data_Validator_Test, 字符串重载拒绝不存在路径)
{
	//指向不存在盘符的 UTF-8 文本
