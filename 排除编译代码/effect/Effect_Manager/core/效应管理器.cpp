...
namespace engine
{
	...
	optional<uint64_t> Effect_Manager::effect_build(shared_ptr<config_event> event)
	{
		...
		//获取新效应记录
		auto* new_record = effect_set.get(ID);
		//获取新效应
		...
		else
		{
			...
			//根据执行优先级降序排序
			sort(effect_set.get(), greater(), &effect_record::priority);
			//重建效应索引映射
			effect_set.map_reset();
	...
	bool Effect_Manager::effect_unload(std::shared_ptr<config_event> event)
	{
		...
		//获取目标效应记录
		auto* target_record = effect_set.get(target_ID);
		//获取效应分组索引
	...
	{
		//执行效应
		for (auto& effect_record:effect_set.get())
		{
			//若记录有效
